...
- 다른 GPC에 속해있는 레지스터간의 데이터 공유는 더 느리다. 이들은 GPU의 L2캐시를 경유해서만 데이터 공유가 가능하기 때문이다. 만일 L2캐시 히트가 실패한다면 역메모리(VRAM)를 참조하는 최악의 상황이 발생하며 이는 가장 느리다.
